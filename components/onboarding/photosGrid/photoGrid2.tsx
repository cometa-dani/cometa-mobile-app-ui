...
export const PhotosGrid2: FC<IPhotosGridProps> = ({
  initialPhotos = [],
  onSelect, mode = 'create',
  maxPhotos = MAX_NUMBER_PHOTOS,
  isHorizontal = false
}) => {
  const { styles, theme } = useStyles(uploadYourPhotosSheet);
  const [userPhotos = [], setUserPhotos] = useState<IPhotoPlaceholder[]>(createPlaceholders(maxPhotos, initialPhotos));
  ...
  const Grid: FC<{ photos?: IPhotoPlaceholder[] }> = useCallback(({ photos = restPhotos }: { photos?: IPhotoPlaceholder[] }) => (
    <ForEach items={generate2DArray(photos)}>
      {(row, i = 0, arr = []) => (
        <View
          key={i}
          style={{ flexDirection: 'row', flex: 1, flexWrap: 'wrap', gap: theme.spacing.sp2 }}>
          <ForEach items={row}>
            {(cell, j = 0) => {
              ...
              return (
                <Pressable
                  key={j}
                  style={({ pressed }) => styles.imageViewer(pressed, !source, isHorizontal)}
                ...
                >
                  ...
                  <View style={styles.imageNum}>
                    <Condition
                      if={isHorizontal}
                      then={
                        <Text style={styles.imageNumText}>{j + (i * arr?.length) + 1}</Text>
                      }
                      else={
...
const uploadYourPhotosSheet = createStyleSheet((theme) => ({
  ...
  image: (isHorizontal: boolean) => ({
    height: '100%',
    flex: 1,
    aspectRatio: isHorizontal ? 0.68 : 1.06,
    borderRadius: theme.radius.sm,
  }),
  imageViewer: (isPressed: boolean, showBorder = true, isHorizontal: boolean) => ({
    position: 'relative',
    height: '100%',
    flex: 1,
    aspectRatio: isHorizontal ? 0.68 : 1.06,
